...
export const getTodayFact = async (userId: number): Promise<Fact | null> => {
  ...
  try {
    const result = await db.query(
      ...
      [format(new Date(), 'yyyy-MM-dd'), userId],
    );
...
export const getFactsCount = async (userId: number): Promise<number> => {
  ...
  try {
    const res = await db.query('SELECT COUNT(*) FROM facts WHERE userId = $1', [
      userId,
    ]);
...
export const getWeekFacts = async (
  userId: number,
  weekStart: Date,
  weekEnd: Date,
): Promise<Fact[]> => {
  ...
  try {
    const res = await db.query(
      'SELECT * FROM facts WHERE created_at::date BETWEEN $1 AND $2 AND userId = $3',
      [weekStart.toISOString(), weekEnd.toISOString(), userId],
    );
